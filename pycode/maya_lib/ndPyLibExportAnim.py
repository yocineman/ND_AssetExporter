...
maya.batchで実行されることを前提にしている。
-------------------------------------------------
abcと異なりndPyLibAnimIOExportContain_mainでエクスポートを行う
-------------------------------------------------
...
def export_anim_main(**kwargs):
    ...
    input_path: 入力ファイルパス
    evaluate: 評価モード 
    frame_handle: フレームハンドルの値
    publish_ver_anim_path: 出力パス
    namespace: 名前空間のリスト
    export_item: エクスポート対象のアイテム
    frame_range: フレームレンジ
    on_maya: Maya上での実行かどうか
    step_value: タイムワープのステップ値
    :return: 出力ファイルのリスト
    ----------------------------------
    1, set_env()を実行して環境変数を設定
    2, 入力ファイルを開く
    3, evaluateモードを設定
    4, ミュートされているアニメーションレイヤーを解除
    5, キャッシュノードを非表示にし、無視するアトリビュートを設定
    6, シーンのフレームレンジを取得
    7, 名前空間のリストを取得
    8, エクスポート対象のノードを取得
    9, アニメーションレイヤーをマージ
    10, ベースアニメーションレイヤーを取得
    11, アニメーションをベイク
    12, キーフレームのないアトリビュートを取得
    13, キーフレームのないアトリビュートにキーフレームを設定
    14, 各種アトリビュートを取得
    15, アトリビュートのロックを解除
    16, Eulerフィルターを適用
    17, タイムワープが有効な場合、タイムワープの設定を行う
    18, アニメーションをベイク
    19, 非表示にしたオブジェクトを再表示
    20, 名前空間ごとにアニメーションをエクスポート
    21, 出力ファイルのリストを返す
    ----------------------------------
    """
    set_env()
