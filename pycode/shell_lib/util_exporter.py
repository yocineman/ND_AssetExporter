...
class outputPathConf(object):
    ...
    def set_char(self, char):
        self._publish_char_path = os.path.join(
            self.shot_path, "publish", self.root_dir, char
        )
        if self.export_type == "camera":
            self._publish_char_path = os.path.join(
                self.shot_path, "publish", self.root_dir
            )
    ...
    def copy_ver2current(self):
        ...
        for ver_file in ver_files:
            ...
            current_file_path = os.path.join(
                self.publish_current_path, ver_file
            ).replace("\\", "/")
            print("####ver_file_path##")
            print(ver_file_path)
            ...
            if os.path.isdir(ver_file_path):
                try:
                    ...
                    try:
                        shutil.copytree(
                            self.publish_ver_path, current_file_path, dirs_exist_ok=True
                        )
                    except:
                        print("copy failed.")
    ...
    def publish_current_anim_path(self):
        self._publish_current_anim_path = os.path.join(
            self.publish_current_path, "anim"
        )
...
class ProjectInfo:
    ...
    def get_camera_rig_info(self):
        ...
        try:
            self.camera_rig_export = project_conf.get("preferences").get(
                "camera_rig_export"
            )
...
class DeadlineMod:
    def __init__(self, **kwargs):
        # jobFile
        self.target_py = (
            "Y:/tool/ND_Tools/DCC/Standalone/{}/pycode/exporter_bridge.py".format(
                TOOLNAME
            )
        )
        # infoFile
    ...
    def job_content(self):
        ...
        job_dict["Name"] = (
            "ND_AssetExporter_{pool}_{shot}{sequence}_{asset_name}".format(
                **self.argsdict
            )
        )  # シーンの情報を入れる
        ...
        job_dict["BatchName"] = "Exporter_{pool}_{shot}{sequence}".format(
            **self.argsdict
        )
        return job_dict
    ...
    def file_maker(self, file_type, file_number):
        ...
        deadline_tmpfile = r"{}\ND_AssetExporter_deadline_{}_{}.job".format(
            self.tmp_dir, file_type, file_number
        )
...
def submit_to_deadlineJobs(jobs, farm="Deadline", version="10"):
    ...
    process = subprocess.Popen(
        command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=devnull
    )
    lines_iterator = iter(process.stdout.readline, b"")
    ...
    def __init__(self, log_table, log_dir, parent=None):
        ...
        for _file in reversed(files):
            if ".txt" in _file:
                ...
                info_set = [str(asset), str(_date), str(_time), str(file_path)]
...
def is_arnold(project):
    ...
    if not os.path.exists("%s\\ND_sgtoolkit_%s" % (toolkit_path, project_name)):
        ...
        project_app_launcher = "Y:\\tool\\ND_Tools\\shotgun\\ND_sgtoolkit_{}\\config\\env\\includes\\app_launchers.yml".format(
            project_name
        )
    try:
